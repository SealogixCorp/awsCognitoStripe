...
export default () => {
  ...
  return (
    ...
    <div class="lg:text-center my-10">
      <h2 class="text-base text-indigo-600 font-semibold tracking-wide uppercase">Flower Architect.</h2>
...
<div className="flex flex-wrap items-start justify-center py-4 pt-0">
  {[...monthlyDeal ,...arrangementDeals].map((deal, key) => (
  <div className="w-full transform  p-2 md:w-1/2 lg:w-1/4 plan-card" key={key}>
          <label className="flex flex-col  transform  rounded-lg shadow-2xl group relative cursor-pointer hover:shadow-2xl hover:bg-blue-500">
            <div className="w-full px-4 py-6 rounded-t-lg card-section-1 border-t-2 border-pink-500">
              ...
              <p className="text-5xl font-bold text-center group-hover:text-white hover:bg-blue-500 text-blue-500">
                ${deal.price.split(".")[0]}.<span className="text-3xl">{deal.price.split(".")[1]}</span>
              </p>
              <p className="text-xs text-center uppercase group-hover:text-white hover:bg-blue-500 text-blue-500">
                {deal.paymentType === "recurring" ? "Monthly" : "One time"}
              </p>
            </div>
        ...
        <div className="flex flex-row items-center justify-self-start">
          ...
          className="text-xl font-semibold mr-4">{deal.title}</h3>
          {/*
      ...
      <hr className="mb-4 w-full border-blue-400"></hr>
            ...
            <ul className={`${deal.id === activeAccordion ? "" : "hidden"} `}>

         {deal.description.map((des, key) => (
          <li className="flex items-start text-white p-2 text-left" key={key}>
        ...
        <p className="ml-3 text-base font-medium ">
          {des}
             ...
             <button className="w-2/3 text-center rounded-lg bg-white px-4 py-2 text-base font-semibold font-display  hover:text-teal-500 focus:outline-none focus:shadow-outline"onClick={(e)=>{
               e.preventDefault();
