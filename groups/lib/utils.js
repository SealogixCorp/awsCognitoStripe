...
exports.errLog = function(req, res, location, err, send) {
  ...
  if (res && res.headerSent) {
    if (send === true && res.headerSent !== true) {
      res.send({ success: false, msg: "Error: at " + location, err: err });
    }
  }
};
...
exports.loadHeaderText = function(req, res, returnData) {
  var headerText = {
    ...
    FLOWER_IMAGES_ON_THIS_SITE_ARE_COPYRIGHTED_AND_FOR_USE_ONLY_IN_CREATING_FLOWER_ARRANGEMENTS_ON_THE_FLOWER_ARCHITECT_COM_WEB_SITE: i18n.__(
      "FLOWER_IMAGES_ON_THIS_SITE_ARE_COPYRIGHTED_AND_FOR_USE_ONLY_IN_CREATING_FLOWER_ARRANGEMENTS_ON_THE_FLOWER_ARCHITECT_COM_WEB_SITE"
    ),
    ...
    SEALOGIX_CORP_ALL_RIGHTS_RESERVED_PATENT_PENDING_US: i18n.__(
      "© 2017_SEALOGIX_CORP_ALL_RIGHTS_RESERVED_PATENT_PENDING_US_14212028"
    ),
    SHOP: i18n.__("SHOP"),
      ...
      User.update(
        ...
        function(err) {
          if (err) {
            ...
            var mna =  encryptor.encrypt(req.user.profile.name);  // .ToDo RMS hashed to match encrypted User profile or data from cognito
                ...
                $addToSet: {
                  mem: {
                    ...
                    eml: crypto
                      ...
                      .update(user.email)
                      .digest("hex"), // hashed
                    encryptedEmail: encryptor.encrypt(req.user.profile.email) // encrypted
                  }
                }
              },
              function(err) {
          ...
          newToken.save(function(err) {
            if (err) {
              ...
              var mailOptions = {
                ...
                subject: i18n.__(
                  "PLEASE_CONFIRM_YOUR_EMAIL_TO_COMPLETE_YOUR_REGISTATION_TO_FLOWERARCHITECT_COM"
                ),
                html:
              ...
              transporter.sendMail(mailOptions, function(err) {
                if (err) {
                  utils.errLog(req, res, "users.postLogin.4", err, true);
                    ...
                    req.flash("errors", {
                      msg: i18n.__(
                        "EMAIL_VERIFICATION_SENT_TO_YOUR_EMAIL_ADDRESS"
                      )
                    });
                    res.redirect("/login");
